...
public class Busqueda {
    ...
    public static void ventana(ListaEnlazada<Archivo> lista, Stage primaryStage) throws IOException {
        if (lista.getLargo() != 0) {
            if (Archivo.ListaArboles.getLargo() != 0) {
                ...
                gridPane.setHgap(18);
                GridPane.setMargin(toolBar2, new Insets(20d, 20d, 10d, 40d));
                ...
                for (int i = 0; i < lista.getLargo(); i++) {
                    ...
                    if (!input.getText().equals("")) {
                        Eventos.abrirArchivoPorPalabra(lista.Obtener(i), input, areaDeTexto);
     ...
     * @param primaryStage Escenario de la ventana
     */
    public static void ventana(ListaEnlazada<Archivo> lista, String[] array, Stage primaryStage) throws IOException {
        if (lista.getLargo() != 0) {
            if (Archivo.ListaArboles.getLargo() != 0) {
                primaryStage.setIconified(true);
                ...
                for (int i = 0; i < lista.getLargo(); i++) {
                    ...
                    if (!input.getText().equals("")) {
                        Eventos.abrirArchivoPorFrase(lista.Obtener(i), input, array, areaDeTexto);
                        if (areaDeTexto.getText() != null) {
                            textos.getChildren().addAll(areaDeTexto);
                            botones.getChildren().addAll(boton);
                            posX += 600;
